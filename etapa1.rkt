...
(define (merge ph1 ph2)
  (cond
    ...
    [else
     (let* ((r1 (ph-root ph1))
            (r2 (ph-root ph2))
            (sub1 (ph-subtrees ph1))
            (sub2 (ph-subtrees ph2)))
       (if (> r1 r2)
           (cons r1 (cons ph2 sub1))
           (if (= r1 r2)
               (cons r1 (cons ph2 sub1))
               (cons r2 (cons ph1 sub2)))))]))
...
(define (two-pass-merge-LR phs)
  (define (pairwise-merge lst)
    (cond
      [(null? lst) '()]
      [(null? (cdr lst)) lst]
      [else (cons (merge (car lst) (cadr lst)) (pairwise-merge (cddr lst)))]))
  (define (merge-all lst)
    (if (null? lst)
        ...
        (let loop ((rest (cdr lst)) (acc (car lst)))
          (if (null? rest)
              acc
              (loop (cdr rest) (merge acc (car rest)))))))
  (if (null? phs)
      empty-ph
      (merge-all (pairwise-merge phs))))
...
(define (two-pass-merge-RL phs) 
  (let ((reversed-phs (reverse-list phs)))
    (two-pass-merge-LR reversed-phs)))
...
(define (tournament-merge phs)
  (cond
    ...
    [else
     (let loop ((lst phs) (merged '()))
       (cond
         [(null? lst) (tournament-merge (reverse merged))]
         [(null? (cdr lst)) (tournament-merge (reverse (cons (car lst) merged)))]
         [else
          (loop (cddr lst) (cons (merge (car lst) (cadr lst)) merged))]))]))
...
(define (ph-del-root ph)
  (if (ph-empty? ph)
      #f
      (let ((sub (ph-subtrees ph)))
        (if (null? sub)
            empty-ph
            (two-pass-merge-LR sub)))))